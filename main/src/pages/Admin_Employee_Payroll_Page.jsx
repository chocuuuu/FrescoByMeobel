...
function AdminEmployeePayrollPage() {
  ...
  const fetchPayrollData = async () => {
    try {
      ...
      // Fetch SSS data
      const sssResponse = await fetch(`${API_BASE_URL}/sss/`, { headers })
      let sssData = []
      if (sssResponse.ok) {
      ...
      const philhealthResponse = await fetch(`${API_BASE_URL}/philhealth/`, { headers })
      let philhealthData = []
      ...
      // Fetch Pag-IBIG data
      const pagibigResponse = await fetch(`${API_BASE_URL}/pagibig/`, { headers })
      let pagibigData = []
  ...
  const handleDeletePayroll = async (employeeId, userId) => {
    ...
    try {
      ...
      if (employee.sssData && employee.sssData.id) {
        deletionPromises.push(
          fetch(`${API_BASE_URL}/sss/${employee.sssData.id}/`, {
            method: "DELETE",
            headers,
          }),
        )
      ...
      if (employee.philhealthData && employee.philhealthData.id) {
        deletionPromises.push(
          fetch(`${API_BASE_URL}/philhealth/${employee.philhealthData.id}/`, {
            method: "DELETE",
            headers,
          }),
        )
      ...
      if (employee.pagibigData && employee.pagibigData.id) {
        deletionPromises.push(
          fetch(`${API_BASE_URL}/pagibig/${employee.pagibigData.id}/`, {
            method: "DELETE",
            headers,
          }),
        )
