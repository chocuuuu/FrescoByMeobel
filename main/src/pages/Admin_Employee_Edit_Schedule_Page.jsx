...
function AdminEmployeeEditSchedulePage() {
      ...
      if (schedule.shift_ids && schedule.shift_ids.length > 0) {
        ...
        shiftDataArray.forEach((shiftData, index) => {
          if (shiftData && matchingDates.includes(shiftData.date)) {
            ...
            console.log(
              `Found shift ${schedule.shift_ids[index]} to remove for date ${shiftData.date}`
            );
          } else {
      ...
      for (let i = 0; i < shiftsToRemove.length; i += chunkSize) {
        ...
        fetch(`${API_BASE_URL}/shifts/batch-delete/`, {
          ...
          .then((res) => {
            if (!res.ok) {
              throw new Error(
                `Batch deletion failed for ${chunk.length} shifts`
              );
            }
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/schedule/?user_id=${queryId}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
          ...
          if (scheduleData.shift_ids && scheduleData.shift_ids.length > 0) {
            ...
            try {
              const shiftResponse = await fetch(
                `${API_BASE_URL}/shift/${scheduleData.shift_ids[0]}/`,
                {
                  headers: {
                    Authorization: `Bearer ${accessToken}`,
                    "Content-Type": "application/json",
  ...
  const handleShiftSelection = async (shift) => {
    ...
    try {
      ...
      if (selectedDayNames.length === 0) {
        console.log(
          "No days selected, but shift type has been set for future use"
        );
      }
    } catch (error) {
  ...
  const handleSaveSchedule = async () => {
    try {
      ...
      if (!selectedShift) {
        alert(
          "Please select a shift type (Morning, Midday, Night, or Custom)."
        );
        return;
      }
      ...
      console.log(
        `After creating shifts, we have ${shiftIds.length} shift IDs`
      );

      // If we still don't have shifts, create a default shift for each day
      if (shiftIds.length === 0) {
        alert(
          "Unable to create shifts automatically. Please try selecting the shift type again."
        );
        return;
      }
...
The calendar has been updated to show the scheduled days.`);
  ...
  const getDayStatusColor = (day) => {
    if (!day.isCurrentMonth)
      return "bg-gray-400 text-white cursor-default pointer-events-none";
