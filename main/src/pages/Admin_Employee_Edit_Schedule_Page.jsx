...
function AdminEmployeeEditSchedulePage() {
  ...
  const isDateInPayrollPeriod = (date) => {
    ...
    return (
      dateObj.isAfter(startDate.subtract(1, "day")) &&
      dateObj.isBefore(endDate.add(1, "day"))
    );
  };
    ...
    const fetchHolidays = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/master-calendar/holiday/`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchPayrollPeriods = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/master-calendar/payrollperiod/`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch payroll periods: ${response.statusText}`
          );
        }
        ...
        const sortedPeriods = data.sort(
          (a, b) =>
            new Date(a.payroll_period_start) - new Date(b.payroll_period_start)
        );
  ...
  const handlePayrollPeriodChange = (e) => {
    ...
    if (selectedPeriod) {
      ...
      if (
        periodStart.month() !== currentDate.month() ||
        periodStart.year() !== currentDate.year()
      ) {
      ...
      if (schedule.id && hasScheduleBeenSaved) {
        updateSchedulePayrollPeriod(
          schedule.id,
          selectedPeriod.payroll_period_start,
          selectedPeriod.payroll_period_end
        );
      }
  ...
  const updateSchedulePayrollPeriod = async (
    scheduleId,
    startDate,
    endDate
  ) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update schedule payroll period: ${response.statusText}`
        );
      }
  ...
  const generateShiftsForDay = async (dayName) => {
    try {
      ...
      console.log(
        `Generating shifts for ${dayName} with shift type: ${shiftType}`
      );
      ...
      console.log(
        `Found ${matchingDates.length} matching dates for ${dayName} within payroll period:`,
        matchingDates
      );
      ...
      for (const dateStr of matchingDates) {
        ...
        if (existingShifts[dateStr]) {
          console.log(
            `Skipping shift creation for ${dateStr} - shift already exists`
          );
          continue;
        }
        ...
        if (shiftResponse.ok) {
          ...
          console.error(
            `Failed to create shift for ${dateStr}:`,
            await shiftResponse.text()
          );
        }
      }
      ...
      matchingDates.forEach((dateStr) => {
        ...
        if (
          !hasSpecialStatus &&
          !(isPastDay && newDayStatus[dateStr] === "absent")
        ) {
  ...
  const addShiftsForDay = async (dayName) => {
    try {
      ...
      if (schedule.id && hasScheduleBeenSaved) {
        ...
        const response = await fetch(
          `${API_BASE_URL}/schedule/${schedule.id}/`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${accessToken}`,
  ...
  const handleDeleteSchedule = async () => {
    ...
    try {
      ...
      holidays.forEach((holiday) => {
        ...
        if (
          dayjs(dateStr).month() === currentDate.month() &&
          dayjs(dateStr).year() === currentDate.year()
        ) {
          newDayStatus[dateStr] =
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      ...
      try {
        const accessToken = localStorage.getItem("access_token");
        const response = await fetch(
          `${API_BASE_URL}/employment-info/${employeeId}/`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchAttendanceData = async () => {
      try {
        ...
        const lastDay = dayjs(new Date(year, month + 1, 0)).format(
          "YYYY-MM-DD"
        );
        ...
        console.log(
          `Fetching attendance data for user ${userId} from ${firstDay} to ${lastDay}`
        );
        ...
        if (response.ok) {
          ...
          const filteredData = data.filter(
            (record) => Number(record.user) === Number(userId)
          );
          console.log(
            `Filtered to ${filteredData.length} records for user ${userId}:`,
            filteredData
          );
          ...
          filteredData.forEach((record) => {
            ...
            console.log(
              `Setting attendance for ${
                record.date
              } to ${record.status.toLowerCase()} (User: ${record.user})`
            );
          });
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      try {
        ...
        if (!userId) {
          console.warn(
            "No user ID found in employee data, using employeeId as fallback"
          );
          setLoading(false);
          return;
        }
        ...
        try {
          const response = await axios.get(
            `${API_BASE_URL}/schedule/?user_id=${userId}`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
          ...
          if (response.data && response.data.length > 0) {
            ...
            if (userSchedules.length > 0) {
              ...
              console.log(
                "Found most recent schedule for this user:",
                scheduleData
              );
              ...
              if (scheduleData.payroll_period_start) {
                ...
                if (payrollPeriods.length > 0) {
                  const matchingPeriod = payrollPeriods.find(
                    (period) =>
                      period.payroll_period_start ===
                      scheduleData.payroll_period_start
                  );
                  if (matchingPeriod) {
              ...
              if (scheduleData.days && Array.isArray(scheduleData.days)) {
                ...
                console.log(
                  "Setting selected days based on schedule:",
                  newSelectedDays
                );
                setSelectedDays(newSelectedDays);
              } else {
                console.warn(
                  "Schedule days is not an array or is undefined:",
                  scheduleData.days
                );
              }
                ...
                try {
                  ...
                  if (mostCommonShift) {
                    ...
                    if (mostCommonShift === "custom") {
                      ...
                      const customShiftDay = Object.entries(newDayShifts).find(
                        ([_, data]) => data.type === "custom"
                      );
                      if (customShiftDay) {
                        const [_, data] = customShiftDay;
                        setCustomShiftStart(
                          convertTimeFormatToTimeString(data.start)
                        );
                        setCustomShiftEnd(
                          convertTimeFormatToTimeString(data.end)
                        );
                      }
                    }
                  }
              ...
              console.log(
                `No schedule found for employee ${employeeId} with user ID ${userId}`
              );
            }
          } else {
  ...
  useEffect(() => {
    if (schedule && schedule.id && schedule.days && schedule.days.length > 0) {
      console.log(
        "Schedule data changed, updating calendar with days:",
        schedule.days
      );
  ...
  useEffect(() => {
    ...
    dayObjects.forEach((day) => {
      if (day.isCurrentMonth) {
        ...
        if (schedule.sickleave === dateStr) {
          ...
          newDayStatus[dateStr] = isInPayrollPeriod
            ? "unselected"
            : "outside-period";
        }
      }
    ...
    if (schedule && schedule.days && schedule.days.length > 0) {
      ...
      console.log(
        ...
        } scheduled dates for ${currentDate.format("MMMM")}`
      );
    }
    ...
    if (
      schedule &&
      (schedule.regularholiday?.length > 0 ||
        schedule.specialholiday?.length > 0)
    ) {
    ...
    if (currentStatus === eventType) {
      ...
      if (eventType === "sickleave") {
        ...
        if (Array.isArray(newSchedule[eventType])) {
          newSchedule[eventType] = newSchedule[eventType].filter(
            (d) => d !== dateStr
          );
        }
      }
  ...
  const handleSaveSchedule = async () => {
    try {
      ...
      if (schedule.id) {
        console.log(
          `Updating existing schedule ${schedule.id} with new shifts:`,
          newShiftIds
        );
        ...
        // Log the data being sent to the server
        console.log(
          "Creating new schedule with data:",
  ...
  const updateCalendarWithSchedule = (savedSchedule, scheduledDays) => {
    ...
    console.log(
      `Found ${scheduledDates.length} dates in ${currentDate.format(
        "MMMM"
      )} that match scheduled days:`,
      scheduledDates
    );

    if (scheduledDates.length === 0) {
      console.warn(
        "No scheduled dates found for the current month. Check if the days array is correct:",
        scheduledDays
      );
      return;
    }
    ...
    setDayStatus((prevStatus) => {
      ...
      Object.keys(newStatus).forEach((dateStr) => {
        ...
        if (dateMonth === String(month).padStart(2, "0")) {
          if (
            ...
            const isInPayrollPeriod = isDateInPayrollPeriod(dateStr);
            newStatus[dateStr] = isInPayrollPeriod
              ? "unselected"
              : "outside-period";
          }
        }
      ...
      scheduledDates.forEach((dateStr) => {
        ...
        const hasSpecialStatus =
          ...
          (savedSchedule.regularholiday &&
            savedSchedule.regularholiday.includes(dateStr)) ||
          (savedSchedule.specialholiday &&
            savedSchedule.specialholiday.includes(dateStr)) ||
          (savedSchedule.nightdiff &&
            savedSchedule.nightdiff.includes(dateStr)) ||
          (savedSchedule.oncall && savedSchedule.oncall.includes(dateStr)) ||
          (savedSchedule.vacationleave &&
            savedSchedule.vacationleave.includes(dateStr));
        ...
        if (!hasSpecialStatus) {
          if (isPastDay && attendanceData[dateStr]) {
            ...
            newStatus[dateStr] =
              attendanceData[dateStr] === "present" ? "attended" : "absent";
          } else {
      ...
      if (
        savedSchedule.regularholiday &&
        Array.isArray(savedSchedule.regularholiday)
      ) {
      ...
      if (
        savedSchedule.specialholiday &&
        Array.isArray(savedSchedule.specialholiday)
      ) {
  ...
  const debugSchedule = () => {
    ...
    console.log(
      `Debug: Found ${scheduledDates.length} dates in ${currentDate.format(
        "MMMM YYYY"
    ...
    const correctlyMarked = scheduledDates.filter(
      (d) => dayStatus[d.date] === "selected"
    ).length;
    console.log(
      `Debug: ${correctlyMarked} out of ${scheduledDates.length} dates are correctly marked as scheduled`
    );
  };
  ...
  const getDayStatusColor = (day) => {
    ...
    switch (status) {
      ...
      case "selected":
        return "bg-blue-200 text-gray-800"; // Blue for selected days
      case "sickleave":
      case "specialholiday":
      case "regularholiday":
      case "vacationleave":
  ...
  useEffect(() => {
    ...
    if (schedule && schedule.days && schedule.days.length > 0) {
      console.log(
        `Month changed to ${currentDate.format(
          "MMMM YYYY"
        )}, updating calendar with schedule days:`,
        schedule.days
      );
      // Small delay to ensure all state is updated
      setTimeout(() => {
            ...
            <div className="bg-[#5C7346] rounded-lg p-6 lg:w-2/3 h-auto max-h-[800px] overflow-auto">
              ...
              <div className="grid grid-cols-7 gap-2 mb-2">
                {[
                  ...
                  <div
                    key={day}
                    className="text-white font-medium text-center py-1 text-sm md:text-lg"
                  >
                  ...
                  return (
                    <div
                      ...
                      ${
                        ...
                        !isInPayrollPeriod && day.isCurrentMonth
                          ? "opacity-50"
                          : ""
                      }`}
