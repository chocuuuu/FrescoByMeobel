    ...
    const fetchEmployeeData = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/api/v1/employment-info/${employeeId}/`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
    ...
    const fetchScheduleData = async () => {
      try {
        ...
        let response = await fetch(
          `http://localhost:8000/api/v1/schedule/?user_id=${userId}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
              "Content-Type": "application/json",
        ...
        if (!response.ok || (await response.json()).length === 0) {
          ...
          response = await fetch(
            `http://localhost:8000/api/v1/schedule/?user_id=${employeeId}`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
                ...
                {selectedShift === "custom" && (
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    ...
                    <div className="flex flex-col">
                      <label className="text-white text-md font-semibold mb-2">
                        Start Time
                      </label>
                      <input
                        type="time"
