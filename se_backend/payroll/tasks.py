...
def calculate_total_deductions(deductions, overtime):
    return (
        sum([
            deductions.sss, deductions.philhealth, deductions.pagibig, deductions.wtax,
            deductions.nowork, deductions.loan, deductions.charges, deductions.msfcloan
        ]) if deductions else 0
    ) + (
        (overtime.total_late if overtime else 0) + (overtime.total_undertime if overtime else 0)
    )
...
def generate_payroll_entries():
    ...
    for salary in salaries:
        ...
        total_deductions = calculate_total_deductions(deductions, overtime)
